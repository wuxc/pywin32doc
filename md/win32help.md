 ...
 **HH_ALINK_LOOKUP** : Looks up one or more Associative link (ALink) names 
 ...
 **HH_CLOSE_ALL** : Closes all windows opened directly or indirectly by the 
 ...
 **HH_DISPLAY_INDEX** : Selects the Index tab in the Navigation pane of the 
 ...
 **HH_DISPLAY_SEARCH** : Selects the Search tab in the Navigation pane of 
 ...
 **HH_GET_LAST_ERROR** : Returns information about the last error that 
 ...
 **HH_GET_WIN_HANDLE** : Returns the handle (hwnd) of a specified window 
 ...
 **HH_HELP_CONTEXT** : Displays a help topic based on a mapped topic ID. 
 ...
 **HH_INITIALIZE** : This command initializes the help system for use and 

must be the first HTML Help command called. It returns a cookie which must 

be used in the **HH_UNINITIALIZE** call. HH_INITIALIZE configures HTML Help 

to run on the same thread as the calling application instead of a 

secondary thread by setting the global property **HH_GPROPID_SINGLETHREAD** to VARIANT_TRUE. Running HTML Help on the same thread as the calling 

application requires the calling application to send messages to HTML Help 

by calling the **HH_PRETRANSLATEMESSAGE** command.

 **file** : Must be None.
 **data** : Ignored.

 **HH_KEYWORD_LOOKUP** : Looks up one or more keywords in a compiled help 
 ...
 **HH_SET_WIN_TYPE** : Creates a new help window or modifies an existing 
 ...
 **HH_TP_HELP_CONTEXTMENU** : Opens a pop-up context menu. Generally used 
 ...
 **HH_TP_HELP_WM_HELP** : Opens a pop-up help topic. Generally used in 
 ...
 **HH_UNINITIALIZE** : This command is called to properly shut down HTML 
